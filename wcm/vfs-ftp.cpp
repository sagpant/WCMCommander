...
void FTPNode::Passive(unsigned &passiveIp, int &passivePort)
{
	...
	s++;
	int ip1 = SKIPINT(s); if (*s!=',') throw int (EFTP_BADPROTO); s++; 
	int ip2 = SKIPINT(s); if (*s!=',') throw int (EFTP_BADPROTO); s++; 
	int ip3 = SKIPINT(s); if (*s!=',') throw int (EFTP_BADPROTO); s++; 
	int ip4 = SKIPINT(s); if (*s!=',') throw int (EFTP_BADPROTO); s++; 
	int port1 = SKIPINT(s); if (*s!=',') throw int (EFTP_BADPROTO); s++; 
	int port2 = SKIPINT(s); if (*s!=')') throw int (EFTP_BADPROTO); s++; 
...
void FTPNode::Connect(unsigned ip, int port, const char *user, const char *password, bool passive)
{
	...
	int rc = ReadCode(); CheckFtpRet(rc);
...
void FTPNode::Ls(ccollect<carray<char> > &list)
{
	OpenData("LIST\r\n");
	list.clear();		
	char buf[4096];
...
FSString FSFtp::StrError(int err)
{
	...
	switch (err) {
	case EFTP_BADPROTO:			str = "protocol error"; break;
	case EFTP_NOTSUPPORTED:			str = "operation is not supported"; break;
	case EFTP_OUTOFRESOURCE_INTERNAL:	str = "out of internal FSFtp resources"; break;
	case EFTP_INVALID_PARAMETER:		str = "invelid parameter"; break;
	
	case EFTP_NOTEXIST:	str = "file not exist"; break;
	case EFTP_EXIST:	str = "file exist"; break;
	
	
	case -3:	str = "Operation timed out."; break;
	
	case -331:	str = "331 User name okay, need password."; break;
	case -332:	str = "332 Need account for login."; break;
	case -350:	str = "350 Requested file action pending further information."; break;
	case -421:	str = "421 Service not available, closing control connection."; break;
	case -425:	str = "425 Can't open data connection."; break;
	case -426:	str = "426 Connection closed; transfer aborted."; break;
	case -450:	str = "450 Requested file action not taken."; break;
	case -451:	str = "451 Requested action aborted. Local error in processing."; break;
	case -452:	str = "452 Requested action not taken."; break;
	case -500:	str = "500 Syntax error, command unrecognized."; break;
	case -501:	str = "501 Syntax error in parameters or arguments."; break;
	case -502:	str = "502 Command not implemented."; break;
	case -503:	str = "503 Bad sequence of commands."; break;
	case -504:	str = "504 Command not implemented for that parameter."; break;
	case -530:	str = "530 Not logged in."; break;
	case -532:	str = "532 Need account for storing files."; break;
	case -550:	str = "550 Requested action not taken."; break;
	case -551:	str = "551 Requested action aborted. Page type unknown."; break;
	case -552:	str = "552 Requested file action aborted."; break;
	case -553:	str = "553 Requested action not taken."; break;
...
static int StringToMode(const char *s)
{
	...
	case 's': m |= S_IXUSR | S_ISUID; break;
	...
	case 's': m |= S_IXGRP | S_ISGID; break;
	...
	case 't': m |= S_IXOTH | S_ISVTX; break;
...
*/
	int mon = GetUnsigned(p0); if (mon>0) mon--;
	int day = GetUnsigned(p0+3); 
	int year = GetUnsigned(p0+6); if (year<70) year += 100;
