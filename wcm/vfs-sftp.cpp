...
FSString FSSftp::StrError(int err) 
{
	...
	case SSH_FX_OK:			s = "No error"; break;
	case SSH_FX_EOF:		s = "End-of-file encountered"; break;
	case SSH_FX_NO_SUCH_FILE:	s = "File doesn't exist"; break;
	case SSH_FX_PERMISSION_DENIED:	s = "Permission denied"; break;
	case SSH_FX_FAILURE:		s = "Generic failure"; break;
	case SSH_FX_BAD_MESSAGE:	s = "Garbage received from server"; break;
	case SSH_FX_NO_CONNECTION:	s = "No connection has been set up"; break;
	case SSH_FX_CONNECTION_LOST:	s = "There was a connection, but we lost it"; break;
	case SSH_FX_OP_UNSUPPORTED:	s = "Operation not supported by the server"; break;
	case SSH_FX_INVALID_HANDLE:	s = "Invalid file handle"; break;
	case SSH_FX_NO_SUCH_PATH:	s = "No such file or directory path exists"; break;
	case SSH_FX_FILE_ALREADY_EXISTS:	s = "An attempt to create an already existing file or directory has been made"; break;
	case SSH_FX_WRITE_PROTECT:	s = "We are trying to write on a write-protected filesystem"; break;
	case SSH_FX_NO_MEDIA:		s = "No media in remote drive"; break;
