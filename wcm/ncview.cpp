...
public:
	...
	void Del(Node *p){ 
		if (p->queuePrev) p->queuePrev->queueNext = p->queueNext; else first = p->queueNext;
		if (p->queueNext) p->queueNext->queuePrev = p->queuePrev; else last = p->queuePrev;
	};
...
bool ViewWin::EventKey(cevent_key* pEvent)
{
	...
	if (pEvent->Type() == EV_KEYDOWN)
	{
		...
		{
			switch (pEvent->Key()) {
			case VK_NEXT:	threadData->SetEvent(ViewerEvent(ViewerEvent::END)); return true;
			case VK_PRIOR:	threadData->SetEvent(ViewerEvent(ViewerEvent::HOME)); return true;
			case VK_RIGHT:	threadData->SetEvent(ViewerEvent(ViewerEvent::RIGHTSTEP)); return true;
			case VK_LEFT:	threadData->SetEvent(ViewerEvent(ViewerEvent::LEFTSTEP)); return true;
			}
		}

		switch (pEvent->Key()) {
		case VK_RIGHT:	threadData->SetEvent(ViewerEvent(ViewerEvent::RIGHT)); break;
		case VK_LEFT:	threadData->SetEvent(ViewerEvent(ViewerEvent::LEFT)); break;
		case VK_DOWN:	threadData->SetEvent(ViewerEvent(ViewerEvent::DOWN)); break;
		case VK_UP:		threadData->SetEvent(ViewerEvent(ViewerEvent::UP)); break;
		
		case VK_HOME:	threadData->SetEvent(ViewerEvent(ViewerEvent::HOME)); break;
		case VK_END:	threadData->SetEvent(ViewerEvent(ViewerEvent::END)); break;
		case VK_NEXT:	threadData->SetEvent(ViewerEvent(ViewerEvent::PAGEDOWN)); break;
		case VK_PRIOR:	threadData->SetEvent(ViewerEvent(ViewerEvent::PAGEUP)); break;
		};
		return true;
...
bool ViewWin::Command(int id, int subId, Win *win, void *data)
{
	...
	switch (subId) {
	case SCMD_SCROLL_LINE_RIGHT:	threadData->SetEvent(ViewerEvent(ViewerEvent::RIGHT)); break;
	case SCMD_SCROLL_LINE_LEFT:	threadData->SetEvent(ViewerEvent(ViewerEvent::LEFT)); break;
	case SCMD_SCROLL_LINE_DOWN:	threadData->SetEvent(ViewerEvent(ViewerEvent::DOWN)); break;
	case SCMD_SCROLL_LINE_UP:	threadData->SetEvent(ViewerEvent(ViewerEvent::UP)); break;

	case SCMD_SCROLL_PAGE_UP: threadData->SetEvent(ViewerEvent(ViewerEvent::PAGEUP)); break;
	case SCMD_SCROLL_PAGE_DOWN: threadData->SetEvent(ViewerEvent(ViewerEvent::PAGEDOWN)); break;
	case SCMD_SCROLL_PAGE_LEFT: threadData->SetEvent(ViewerEvent(ViewerEvent::PAGELEFT)); break;
	case SCMD_SCROLL_PAGE_RIGHT: threadData->SetEvent(ViewerEvent(ViewerEvent::PAGERIGHT)); break;
...
void *VSThreadFunc(void *ptr) 
{
	...
	try {
		...
		try { data->err = new_char_str(ex->message()); } catch (cexception *x) { x->destroy(); }
		ex->destroy();
	} catch (...) {
		try { data->err = new_char_str("BOTVA: unhabdled exception: void *VSThreadFunc(void *ptr) "); } catch (cexception *x) { x->destroy(); }
	}
