...
protected:
	...
	bool __Select(bool sRead, int timeoutSec = -1) //ret false on timeout
	{
		...
		FD_ZERO(&fs);
		FD_SET(sock, &fs);
		struct timeval tv; if (timeoutSec>=0) { tv.tv_sec = timeoutSec; tv.tv_usec = 0; }
...
public:
	...
	void SetNonBlock(bool yes = true){
		...
		if (yes) flags |= O_NONBLOCK; else flags &= ~long(O_NONBLOCK);
...
public:
	...
	void Connect(unsigned ip, int port)
	{
		...
		try {
			sock.Bind();
			sock.SetNonBlock();
			try { sock.Connect(ip, port); } catch (int e) {
#ifdef _WIN32
