...
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   19.08.2012 íóæíî ïåğåğàáîòàòü ccollect::del
   ïğè óäàëåíèè îáúåêòîâ îíè ôàêòè÷åñêè ìîãóò íå óäàëèòüñÿ èç çà øàãà (stop)
   ıòî ÷ğåâàğî (êîñÿê ïîâëèÿë íà îøèáêó ñ Layout, ïîêà ıòà äûğà â Layout çàòêíóòà ïğåäâàğèòåëüíûì çàíóëåíèåì ïåğåä óäàëåíèåì)
*/
...
namespace wal
{
	...
	*/
	...
	public:
		...
		void clear(); //ÕÎÉŞÔÏÖÁÅÔ ÏÂßÅËÔ
		void drop(); //ÏÔÃÅĞÌÑÅÔ ÏÂßÅËÔ
		carray& operator = ( T* p );
...
//!!!ïğîáëåìà ïğè óäàëåíèè (åñëè óäàëÿòü ïîñëåäíèé ıëåìåíò, òî ôàêòè÷åñêè îí íå âñåãäà óäàëÿåòñÿ, à òîëüêî ïğè ïåğåõîäå øàãà (ñ îáúåêòàìè äåğüìî ïîëó÷àåòñÿ))
	...
	template <class T, int step> inline void ccollect<T, step>::insert( int n, const T& a )
	{
		...
		for ( int i = cnt - 2; i >= n; i-- ) //cnt-1 Ğ·Ğ°Ğ¼ĞµĞ½ĞµĞ½Ğ¾ Ğ½Ğ° cnt-2 25.08.2011 Ğ±Ñ‹Ğ» ĞºĞ¾ÑÑĞº
		{
			m_data[i + 1] = m_data[i];
		}
...
#define FloatTableParam F_TP
	...
	/*
	   LT
	      ...
	      unsigned intKey()
	      ŞÌÅÎ LT *next
	      ËÏÎÓÔÒÕËÔÏÒ ËÏĞÉÒÏ×ÁÎÉÑ

	   KT - ÔÉĞ ËÌÀŞÁ
	      ÄÏÌÖÅÎ ÉÍÅÔØ operator unsigned()
	      ÄÏÌÖÅÎ ÓÒÁ×ÎÉ×ÁÔØÓÑ ÎÁ ==

	   IC - true
	      ÅÓÌÉ ÓÎÁŞÁÌÁ ÓÒÁ×ÎÉ×ÁÔØ intKey(), Á ÚÁÔÅÍ key()
	      ÉÎÁŞÅ ÔÏÌØËÏ key()

	      (ÅÓÌÉ KT ÃÅÌÏŞÉÓÌÅÎÎÙÊ É ÒÁ×ÅÎ intKey() ÔÏ ÓÒÁ×ÎÉ×ÁÔØ 2 ÒÁÚÁ ÎÅÚÁŞÅÍ
	       (ÓĞÏÒÎÙÊ ÍÏÍÅÎÔ))
	*/
	...
	inline LT* internal_hash<LT, KT, IC, P>::find( unsigned intKey, const KT& key )
	{
		if ( tableSize > 0 )
		{
			for ( LT* p = table[intKey % tableSize]; p ; p = p->next )
			{
				...
				{
					if ( intKey == p->intKey() &&
					     p->key() == key ) //ĞÏÒÑÄÏË ÏĞÅÒÁÎÄÏ× × == ×ÁÖÅÎ
					{
						return p;
					}
				}
				...
				{
					if ( p->key() == key ) //ĞÏÒÑÄÏË ÏĞÅÒÁÎÄÏ× × == ×ÁÖÅÎ
					{
						return p;
					}
				}
	...
	bool internal_hash<LT, KT, IC, P>::del( unsigned intKey, const KT& key, bool shrink )
	{
		...
		for ( p = table.data() + n; *p; p = &( p[0]->next ) )
			...
			//ĞÏÒÑÄÏË ÏĞÅÒÁÎÄÏ× ÓÒÁ×ÎÅÎÉÊ ×ÁÖÅÎ
			//ŞÔÏÂÙ ÏĞÒÅÄÅÌÑÔØ ÔÏÌØËÏ ×ÓÔÒÏÅÎÎÙÊ ÏĞÅÒÁÔÏÒ == × KT
			if ( IC )
			{
	...
	*/
	...
	/*
	   ÔÁÂÌÉÃÁ, ÇÄÅ ÉÎÄÅËÓÏÍ Ñ×ÌÑÅÔÓÑ ÃÅÌÏÅ ŞÉÓÌÏ (IT)
	   IT - ÃÅÌÏÅ ŞÉÓÌÏ, ÉÌÉ ÔÉĞ, ĞÒÉ×ÏÄÉÍÙÊ Ë unsigned (), ËÏĞÉÒÕÅÔÓÑ É ÓÒÁ×ÎÉ×ÁÅÔÓÑ ÎÁ ==
	*/
