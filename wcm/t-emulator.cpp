...
//надо сделать и вписать таблицы
	switch (c){
	case '0': return decGraphic;
	case 'A': return 0; //UK  
	case 'B': return 0; //USASCII
	case '4': return 0; //Dutch
	
	case 'C': return 0; //Finish
	case '5': return 0; 
	
	case 'R': return 0; //French
	case 'Q': return 0; //French Canadian
	case 'K': return 0; //Germab
	case 'Y': return 0; //Italic
	
	case 'E': return 0; //Norvegian Danish
	case '6': return 0;
	
	case 'Z': return 0; //Spanish
	
	case 'H': return 0; //Swedish
	case '7': return 0;
...
void Emulator::EraseDisp(int mode)
{
	int i;
	switch (mode) {
	case 0:	for (i=0; i <= _rows - _cursor.row - 1; i++) _screen->SetLineChar(i, 0, _cols, _attr.Color() + ' '); break;
	case 1: for (i=_rows - _cursor.row - 1; i<_rows; i++) _screen->SetLineChar(i, 0, _cols, _attr.Color() + ' '); break;
	case 2: for (i=0; i<_rows; i++) _screen->SetLineChar(i, 0, _cols, _attr.Color() + ' '); break;
	}
}
...
void Emulator::Append(char ch)
{
	...
	case ST_ESCAPE:
		switch (ch) {
		case '[': _state = ST_CSI; _N.Reset(); return;
		case '(': _state = ST_G0_CHSET; return;
		case ')': _state = ST_G1_CHSET; return;
		case '*': _state = ST_G2_CHSET; return;
		case '+': _state = ST_G3_CHSET; return;
		case ']': _state = ST_TP_NUM; _N.Reset(); return;
...
//		case '>': _keypad = K_NORMAL; break;
		...
		case 'M': RI(); break;
		case 'N': 	DBG("DBG: esc N (SS2)\n"); break;
		case 'O': 	DBG("DBG: esc O (SS3)\n"); break;
		case 'P': 	DBG("DBG: esc P (DCS)\n"); break;
		case 'V':	DBG("DBG: esc V (SPA)\n"); break;
	...
	case ST_CSI:
		...
		switch (ch) {
		case '?': _state = ST_DECPM; _N.Reset(); return;
		};
			
		if (_N.ReadList(ch)) return;
		
		switch (ch) {
		case 'd': SetCursor((_N[0]?_N[0]:1) - 1, _cursor.col); break;
		case 'f': SetCursor((_N[0]?_N[0]:1) - 1, (_N[1]?_N[1]:1) - 1); break;
...
//printf("_N[%i] = %i\n", i, _N[i]);
					switch (_N[i]) {
					case 0:  _attr.SetNormal(); break;
					case 1:  _attr.bold = true; break;
					case 4:  _attr.underscore = true; break;
					case 5:  _attr.blink = true; break;
					case 7:  _attr.inverse = true; break;
					//case 8: set invisible
					case 22: _attr.bold = false; break;
					case 24: _attr.underscore = false; break;
...
//printf("ECH\n");				
		...
		case 'h':
			{
				for (int i = 0; i<= _N.count; i++)
					switch (_N[i]) {
					case 1: _keypad = K_APPLICATION; break;
					case 7: 
						_wrap = true; 
						break;
	...
	case ST_G0_CHSET: _attr.G[0] = GetCSetTable(ch); break;
	case ST_G1_CHSET: _attr.G[1] = GetCSetTable(ch); break;
	case ST_G2_CHSET: _attr.G[2] = GetCSetTable(ch); break;
	case ST_G3_CHSET: _attr.G[3] = GetCSetTable(ch); break;
