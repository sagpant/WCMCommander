...
char *VSearcher::Search(char *s, char *end, int *fBytes)
{
	char *ret = 0;
	switch (mode) {
	case 1: ret = VSearchStr(s, end, sBigList.ptr(),fBytes); break;
	case 2: ret = VSearchStr(s, end, sList.ptr()); if (ret && fBytes) *fBytes = sList.count()-1; break;
	case 3: ret = VSearchStr(s, end, bytes.ptr(), bytes.count()); if (ret && fBytes) *fBytes = bytes.count(); break;
	};
	return ret;
}
...
int VSearcher::MinLen()
{
	...
	case 2: return sList.count()-1;
	case 3: return bytes.count(); 
	};
	return 0;
}
