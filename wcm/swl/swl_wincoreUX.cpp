...
void InitTrueColor() //called if visual class is TrueColor
{
	...
	if (	visualInfo.red_mask == 0x0000FF && 
		visualInfo.green_mask == 0x00FF00 && 
...
void AppInit()
{
	...
	case TrueColor: InitTrueColor(); break;
	
	case GrayScale:     	//!!! 
	case StaticGray: ;      //!!!
	case StaticColor:       
	case PseudoColor: InitPseudoColor(); break;
	
	
	case DirectColor: 	throw_msg("DirectColor visual class is not supported");
	default:
...
static void DoKeyEvent(int type, Win *w, KeySym ks, unsigned km,  unicode_t ch)
{
	...
	case XK_c: ks = XK_C; break;
	case XK_d: ks = XK_D; break;
	case XK_e: ks = XK_E; break;
	case XK_f: ks = XK_F; break;	
	case XK_g: ks = XK_G; break;
	case XK_h: ks = XK_H; break;
	case XK_i: ks = XK_I; break;
	case XK_j: ks = XK_J; break;
	case XK_k: ks = XK_K; break;
	case XK_l: ks = XK_L; break;
	case XK_m: ks = XK_M; break;
	case XK_n: ks = XK_N; break;
	case XK_o: ks = XK_O; break;
	case XK_p: ks = XK_P; break;
	case XK_q: ks = XK_Q; break;
	case XK_r: ks = XK_R; break;
	case XK_s: ks = XK_S; break;
	case XK_t: ks = XK_T; break;
	case XK_u: ks = XK_U; break;
	case XK_v: ks = XK_V; break;
	case XK_w: ks = XK_W; break;
...
static bool ReadCharProp(Window winId, Atom prop, CharQueue *buf)
{
	...
	if (retType != atom_INCR) 
	{
		...
		switch (retFormat) {
		case 8: size = retNItems; break;
		case 16: size = retNItems*2; break;
		case 32: size = retNItems*4; break;
		default: size = -1; 
		}
...
static void _ClipboardRequest(XEvent *event)
{
	...
	*/
	...
	if (event->xselectionrequest.target == atom_UTF8) ClipboardToCharQueueAsUtf8(&q, &clipboardText);
		...
		if (event->xselectionrequest.target == atom_STRING) ClipboardToCharQueueAsLatin1(&q, &clipboardText);
		else {
...
: type(-1), data(0)
{
	...
	const char *w;
	switch (weight) {
		case Normal: w = "medium"; break;
		case Light: w = "light"; break;
		case Bold: w = "bold"; break;
		default: w = "*"; 
	}
	const char *slant = "r";
