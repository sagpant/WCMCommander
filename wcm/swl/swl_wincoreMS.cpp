...
static LRESULT CALLBACK WProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message) {
		case WM_CREATE: return 0; //return 1;
...
//	break;
		...
		case WM_GETMINMAXINFO: return 1;
		...
		case WM_MOUSEMOVE:		return MouseEvent(EV_MOUSE_MOVE,0,win,wParam,lParam) ? 0 : 1;
		
		case WM_LBUTTONDOWN:	return MouseEvent(EV_MOUSE_PRESS,	MB_L, win, wParam, lParam) ? 0 : 1;
		case WM_LBUTTONUP:		return MouseEvent(EV_MOUSE_RELEASE,	MB_L, win, wParam, lParam);
		case WM_LBUTTONDBLCLK :	return MouseEvent(EV_MOUSE_DOUBLE,	MB_L, win, wParam, lParam);

		case WM_MBUTTONDOWN:	return MouseEvent(EV_MOUSE_PRESS,	MB_M, win, wParam, lParam);
		case WM_MBUTTONUP:		return MouseEvent(EV_MOUSE_RELEASE,	MB_M, win, wParam, lParam);
		case WM_MBUTTONDBLCLK :	return MouseEvent(EV_MOUSE_DOUBLE,	MB_M, win, wParam, lParam);

		case WM_RBUTTONDOWN:	return MouseEvent(EV_MOUSE_PRESS,	MB_R, win, wParam, lParam);
		case WM_RBUTTONUP:		return MouseEvent(EV_MOUSE_RELEASE,	MB_R, win, wParam, lParam);
		case WM_RBUTTONDBLCLK :	return MouseEvent(EV_MOUSE_DOUBLE,	MB_R, win, wParam, lParam);
		...
		case WM_SIZE: return win->Event(&cevent_size(cpoint (lParam&0xFFFF, (lParam>>16)&0xFFFF)));
		case WM_MOVE: return win->Event(&cevent_move(cpoint (lParam&0xFFFF, (lParam>>16)&0xFFFF)));
...
{
	switch (type) {
	case SHOW_ACTIVE: ::ShowWindow(handle, SW_SHOW); break;
	case SHOW_INACTIVE: ::ShowWindow(handle, SW_SHOWNA); break;
	};
	SetState(S_VISIBLE); 
}
...
:external(false)
{
	...
	//lf.lfOrientation 
	switch (weight) {
		case Bold: lf.lfWeight = FW_BOLD; break;
		case Normal:lf.lfWeight = FW_NORMAL; break;
		case Light:lf.lfWeight = FW_LIGHT; break;
		default: lf.lfWeight = FW_DONTCARE; break;
	};
...
carray<char> cfont::LogFontToUru(LOGFONT &lf)
{
	...
	switch (lf.lfWeight){
	case FW_BOLD: uri.append('B'); break;
	case FW_NORMAL: uri.append('N'); break;
	case FW_LIGHT: uri.append('L'); break;
	};
...
void cfont::UriToLogFont(LOGFONT *plf, const char*uri)
{
	...
	if (*s == '-') 
	{
		...
		switch (weight) {
		case 'B': plf->lfWeight = FW_BOLD; break;
		case 'N': plf->lfWeight = FW_NORMAL; break;
		case 'L': plf->lfWeight = FW_LIGHT; break;
		default:  plf->lfWeight = FW_DONTCARE; break;
		};
