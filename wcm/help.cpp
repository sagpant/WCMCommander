...
begin:
	...
	while (true) 
	{
		...
		if (cchar == '/')
		{
			...
			switch (cchar) {
			case 'n': 
				tok = TOK_NL; NextChar(); return tok; 
				break;
			case 'l': tok = TOK_ALEFT; NextChar(); return tok; 
			case 'r': tok = TOK_ARIGHT; NextChar(); return tok; 
			case 'c': tok = TOK_ACENTER; NextChar(); return tok; 
			case 'w': tok = TOK_AWIDTH; NextChar(); return tok; 
			default: word.append(cchar); NextChar(); continue;
			}
		...
		if (cchar == '@')
		{
			...
			switch (cchar) {
			case 't': tok = TOK_TAB; NextChar(); return tok;  
			case 'e': tok = TOK_TAB_END; NextChar(); return tok; 
			case 'n': tok = TOK_TAB_NL; NextChar(); return tok;  
			case 'c': tok = TOK_TAB_NF; NextChar(); return tok; 
			default: word.append(cchar); NextChar(); continue;
			}
...
cptr<HelpNode> HelpParzer::Parze()
{
	...
	while (true) {
		...
		case '}': 
			...
			break;
		case TOK_SPACE: pPar->Append( new HelpNodeSpace(this->currentBData.style, 1) ); break;
		case TOK_V: pPar->Append( new HelpNodeV(this->currentBData.style, num) ); break;
		case TOK_H: pPar->Append( new HelpNodeH(this->currentBData.style, num) ); break;
		case TOK_WORD: pPar->Append( new HelpNodeWord(this->currentBData.style, word.ptr(), 0) ); break;
		case TOK_NL: 
			if (pPar->Count()>0) pList->Append(paragraph);
			paragraph = pPar = new HelpNodeParagraph(currentBData.style);
			break;

		case TOK_ALEFT:	  pPar->SetAlign(HelpNodeParagraph::ALIGN_LEFT); break;
		case TOK_ARIGHT:  pPar->SetAlign(HelpNodeParagraph::ALIGN_RIGHT); break;
		case TOK_ACENTER: pPar->SetAlign(HelpNodeParagraph::ALIGN_CENTER); break;
		case TOK_AWIDTH:  pPar->SetAlign(HelpNodeParagraph::ALIGN_WIDTH); break;
...
static cptr<HelpNode> GetHelpNode( const char *theme, cstrhash<HelpStyle> *pStyles)
{
	...
	if (!ptext) 
	{
		...
		if (!strcmp(theme, "edit")) ptext = helpData_edit;
		else
		if (!strcmp(theme, "view")) ptext = helpData_view;
	}
...
bool HelpWin::Command(int id, int subId, Win *win, void *data)
{
	if (id == CMD_SCROLL_INFO) {
		switch (subId) {
		case SCMD_SCROLL_LINE_UP: MoveYOffset(yOffset-10); break; 
		case SCMD_SCROLL_LINE_DOWN:MoveYOffset(yOffset+10); break; 
		case SCMD_SCROLL_PAGE_UP: MoveYOffset(yOffset-helpRect.Height()); break; 
		case SCMD_SCROLL_PAGE_DOWN:MoveYOffset(yOffset+helpRect.Height()); break; 
		case SCMD_SCROLL_LINE_LEFT: MoveXOffset(xOffset-10);break;
		case SCMD_SCROLL_LINE_RIGHT:MoveXOffset(xOffset+10);break;
		case SCMD_SCROLL_PAGE_LEFT: MoveXOffset(xOffset - helpRect.Width());break;
		case SCMD_SCROLL_PAGE_RIGHT:MoveXOffset(xOffset + helpRect.Width());break;
...
bool HelpWin::EventKey(cevent_key* pEvent)
{
	...
	{
		switch (pEvent->Key()) {
		case VK_DOWN: MoveYOffset(yOffset+10); break;
		case VK_UP: MoveYOffset(yOffset-10); break;
		case VK_END: MoveYOffset(dataHeight-helpRect.Height()); break;
		case VK_HOME: MoveYOffset(0); break;

		case VK_PRIOR: MoveYOffset(yOffset-helpRect.Height()); break;
		case VK_NEXT: MoveYOffset(yOffset+helpRect.Height()); break;
